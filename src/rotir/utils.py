...
def matrix_transform_coordinate2image(matrix, L):
    ...
    image_matrix = torch.Tensor(
        [[cos / ps, sin / ps, x], [-sin / ps, cos / ps, y], [0, 0, 1]]
    )
    return image_matrix
...
def matrix_calculation_function(
    output,
    angle="Auto",
    threshold=0.3,
    freeze_scale=False,
    ...
    if isinstance(threshold, (float, int)):
        threshold = (
            torch.tensor([threshold]).repeat(batch_size).float().to(device)
        )  # .view(-1, 1, 1)
    assert (
        len(threshold) == batch_size
    ), "threshold length {} not match to output size {}".format(
        len(threshold), batch_size
    )
    ...
    conf = (
        output["score_map"][:, :-1, :-1] if reshape_map else output["score_map"]
    ...
    for i in range(batch_size):
        # add score
        score = (
            output["score_map"][i, :-1, :-1] if reshape_map else output["score_map"][i]
        )[conf[i]]
        ...
        if angle == "Auto":
            ...
            if alpha[max_order] < -0.75 * torch_pi and alpha.max() > 0.75 * torch_pi:
                ...
                alpha = alpha + (alpha.min() < -0.75 * torch_pi).float() * (
                    2 * torch_pi
                )

            alpha = (
                select_alpha(
                    alpha - alpha[max_order], s=score, t=torch_pi / 10, base=0.9
                )
                + alpha[max_order]
            )

            scale = (
                output["scale_map"][i][conf[i]].mean().exp()
                if not freeze_scale
        ...
        coordinate_1 = (
            torch.stack([hx, hy, torch.ones_like(hx)], dim=0)
            .repeat(1, torch.ceil(longest / len(hx)).int().item())[:, :longest]
            .float()
        )
        coordinate_2 = torch.cat(
            [
                torch.stack([wx, wy], dim=0).float()
                + (output["trans_map"][i][conf[i]].permute(1, 0) if add_trans else 0),
                torch.ones_like(wx).unsqueeze(0).float(),
            ],
            dim=0,
        )
        coordinate_2 = coordinate_2.repeat(
            1, torch.ceil(longest / len(hx)).int().item()
        )[:, :longest]
        ...
        if angle is None:
            angle_scale_matrix = torch.cat(
                [
                    torch.cat(
                        [
                            torch.cos(translation[:, :1, :]),
                            torch.sin(translation[:, :1, :]),
                            translation[:, 1:2, :],
                        ],
                        dim=-1,
                    ),
            ...
            matrix = torch.cat(
                [torch.cat([angle_scale_matrix, translation], dim=2), bottom], dim=1
            )
    ...
    for i in range(batch_size):
        affine_matrix_list.append(
            matrix_transform_coordinate2image(affine_matrix[i], L)
        )

    affine_matrix = torch.stack(affine_matrix_list, dim=0)
    if coordinate:
        matches = torch.cat(
            [coordinate_2[:, :2, :], coordinate_1[:, :2, :]], dim=1
        ).permute(0, 2, 1)[:, :, torch.arange(3, -1, -1).long()]
